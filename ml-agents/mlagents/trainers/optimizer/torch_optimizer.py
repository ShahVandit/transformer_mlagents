...
class TorchOptimizer(Optimizer):
    ...
    def get_trajectory_value_estimates(
        ...
        # Check if the critic has memory and initialize if needed
        memory = None
        if self.critic.memory_size > 0:
            if agent_id in self.critic_memory_dict:
                memory = self.critic_memory_dict[agent_id]
            else:
                memory = torch.zeros((1, 1, self.critic.memory_size))
        ...
        with torch.no_grad():
            # --- START OF CORRECTION ---
            # Use the critic's memory_size to detect if it's a stateful model.
            # This is more general than the old LSTM-specific 'use_recurrent' flag
            # and works correctly for our transformer.
            if self.critic.memory_size > 0:
                (
                    value_estimates,
                    all_next_memories,
        ...
        next_value_estimate, _ = self.critic.critic_pass(
            next_obs_tensors, next_memory, sequence_length=1
        )
