...
FEATURE_COLUMNS = (
    [f"mean_{x}" for x in ids.STATE_VITALS]
    + [f"mean_{x}" for x in ids.TARGET_LABS]
    + [f"std_{x}" for x in ids.TARGET_LABS]
...
def _model(spec, loss="squared_error", quantile=None, seed=cfg.SEED):
    kw = dict(
        loss=loss, max_iter=200, learning_rate=0.06,
...
class TargetLabForecaster:
    ...
    def fit(self, train, val):
        ...
        for lab_idx, lab in enumerate(ids.TARGET_LABS):
            ...
            for spec in cfg.FORECAST_RESIDUAL_CANDIDATES:
                ...
                rmse = float(np.sqrt(np.mean((yva[va_keep] - pred) ** 2)))
                candidates.append({"name": spec["name"], "validation_rmse": rmse})
                if best is None or rmse < best[0]:
                    best = (rmse, copy.deepcopy(spec), mdl)

            _, spec, mean_model = best
            ...
            mean_delta = mean_model.predict(Xva[va_keep])
            ...
            ratio = np.abs(yva[va_keep] - mean_delta) / raw_std
            interval_scale = float(
                np.quantile(ratio, 0.90) / 1.6448536269514722)
            interval_scale = float(np.clip(interval_scale, 0.5, 5.0))
            ...
            self.bounds_[lab] = (
                float(np.quantile(observed, 0.001)),
    ...
    def transform(self, df, training=False):
        ...
        for lab in ids.TARGET_LABS:
            if training:
                ...
                delta = self.models_[lab].predict(X)
    ...
    def state_dict(self):
        return {
            ...
            "interval_scale": self.interval_scale_, "bounds": self.bounds_,
