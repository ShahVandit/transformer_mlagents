...
def main():
    ...
    for lam in args.lambdas:
        ...
        print(f"  val ep_rew_mean={val_summary['ep_rew_mean']:+.3f}  "
              f"ep_rew_std={val_summary['ep_rew_std']:.3f}  "
              f"draws/day={val_summary['draws_per_patient_day']:.3f}  "
    ...
    report = ["# Joint d3rlpy CQL policy family\n\n",
              "| lambda | ep_rew_mean | ep_rew_std | draws/day | coverage |\n",
              "|---|---:|---:|---:|---:|\n"]
    for row in rows:
        report.append(f"| {row['lambda']} | {row['ep_rew_mean']:+.3f} | "
                      f"{row['ep_rew_std']:.3f} | "
